...
> Condition affected transcription fluency such that it was more fluent in the standard condition, F(1, 197) = 110.14, MSE = 2796.13, p < 0.001, d = 1.50. While there were more words typed in the standard condition, F(1, 198) = 6.35, MSE = 14329.49, p = 0.013, d = 0.36

```{r}

rs1 = summary(aov(Transcription_Fluency ~ Condition, d.tidy)); rs1
effect = cohensD(Transcription_Fluency ~ Condition, data = d.tidy); effect

rs2 = summary(aov(Number_of_Words ~ Condition, data = d.tidy)); rs2
effect2 = cohensD(Number_of_Words ~ Condition, data = d.tidy); effect2
```

> essays in the delay condition contained longer words (letters and syllables per word), Fs > 4.17, ps < 0.043, ds > 0.28. There were no other differences (see Table 1).

![](table1.png)

```{r}
rs3 = summary(aov(Letters_Per_Word ~ Condition, data = d.tidy)); rs3
effect3 = cohensD(Letters_Per_Word ~ Condition, data = d.tidy); effect3

rs4 = summary(aov(Syllables_Per_Word ~ Condition, data = d.tidy)); rs4
effect4 = cohensD(Syllables_Per_Word ~ Condition, data = d.tidy); effect4

rs5 = summary(aov(Sentence_count ~ Condition, data = d.tidy)); rs5
effect5 = cohensD(Sentence_count ~ Condition, data = d.tidy); effect5

rs6 = summary(lm(words_per_sentence ~ Condition, data = d.tidy)); rs6
effect6 = cohensD(words_per_sentence ~ Condition, data = d.tidy); effect6

rs7 = summary(lm(paragraph_count ~ Condition, data = d.tidy)); rs7
effect7 = cohensD(paragraph_count ~ Condition, data = d.tidy); effect7
```
...
```{r}
rs8 = summary(aov(Type.Token_Ratio ~ Condition, data = d.tidy)); rs8
effect8 = cohensD(Type.Token_Ratio ~ Condition, data = d.tidy); effect8
```

> and vocd-D (marginally), F(1, 198) = 3.29, MSE = 264.62, p = 0.071, d = 0.26, such that they were higher in the delay condition. 
 
```{r} 
rs9 = summary(aov(vocdD_Lexical_Diversity ~ Condition, data = d.tidy)); rs9
effect9 = cohensD(vocdD_Lexical_Diversity ~ Condition, data = d.tidy); effect9
```
...
```{r}
rs10 = summary(aov(Measure_of_Textual_Lexical_Diversity_MTLD ~ Condition, data = d.tidy)); rs10
effect10 = cohensD(Measure_of_Textual_Lexical_Diversity_MTLD ~ Condition, data = d.tidy); effect10
```
...
```{r}
rs11 = summary(aov(Word_Frequency_all_words_log ~ Condition, data = d.tidy)); rs11
effect11 = cohensD(Word_Frequency_all_words_log ~ Condition, data = d.tidy); effect11
```

> raw word frequency-content words, F(1, 198) = 4.74, MSE = 0.02, p = 0.031, d = 0.31 (see Table 2). 

```{r}
rs12 = summary(aov(Word_Frequency_content_words_raw ~ Condition, data = d.tidy)); rs12
effect12 = cohensD(Word_Frequency_content_words_raw ~ Condition, data = d.tidy); effect12
```

> The results were similar when lexical diversity and word frequency from TAACO and TAALES were used (see Table 3) (from Medimorec et al., p. 30).

![](table3.png)

```{r}
rs13 = summary(aov(type.token_ratio_TAACO ~ Condition, data = d.tidy)); rs13
effect13 = cohensD(type.token_ratio_TAACO ~ Condition, data = d.tidy); effect13
```

```{r}
rs14 = summary(aov(SUBTLEXusFreq_ALL_Words_Log ~ Condition, data = d.tidy)); rs14
effect14 = cohensD(SUBTLEXusFreq_ALL_Words_Log ~ Condition, data = d.tidy); effect14
```

```{r}
rs15 = summary(aov(BNCWrittenFreq_AllWords_Log ~ Condition, data = d.tidy)); rs15
effect15 = cohensD(BNCWrittenFreq_AllWords_Log ~ Condition, data = d.tidy); effect15
```

```{r}
rs16 = summary(aov(SUBTLEXusFreq_Content_Words_Log ~ Condition, data = d.tidy)); rs16
effect16 = cohensD(SUBTLEXusFreq_Content_Words_Log ~ Condition, data = d.tidy); effect16
```

```{r}
rs17 = summary(aov(BNCWrittenFreq_Content_Words_Log ~ Condition, data = d.tidy)); rs17
effect17 = cohensD(BNCWrittenFreq_Content_Words_Log ~ Condition, data = d.tidy); effect17
```
